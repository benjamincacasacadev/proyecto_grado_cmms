{{Form::open(array('action'=>array('StFormController@destroy',code($form->id)),'method'=>'delete', 'onsubmit'=>'btnSubmitDelete.disabled = true; return true;'))}}
    ...
    <div class="modal-body text-center py-4">
        @if ($count == 0)
            ...
            <div class="text-muted">
                ¿Está seguro de eliminar el formulario <b>{{ $form->name }}</b>?
            </div>
        @else
            ...
            <h3>No puede eliminar el formulario <b>{{ $form->name }}</b> </h3>
            <div class="text-muted">
                El formulario tiene {{ $count }} @if( $count == 1) orden de trabajo asociada. @else órdenes de trabajo asociadas. @endif
            </div>
        @endif
    ...
    <div class="modal-footer">
        <div class="w-100">
            <div class="row">
                ...
                @if ($count == 0)
                    <div class="col">
                        <button type="submit" class="btn btn-danger w-100" name="btnSubmitDelete">Eliminar</button>
                    </div>
                @endif
            </div>
        </div>
