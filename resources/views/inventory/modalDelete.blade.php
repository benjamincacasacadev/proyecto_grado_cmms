{{Form::open(array('action'=>array('InventoryController@destroy',code($item->id)),'method'=>'delete', 'onsubmit'=>'deleteboton.disabled = true; return true;'))}}
    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
    <div class="modal-status @if( $item->getCantDetails() == 0 ) bg-red @else bg-yellow @endif"></div>
    <div class="modal-body text-center py-4">
        ...
        @if( $item->getCantDetails() == 0)
            ...
            <div class="text-muted">
                ¿Está seguro de eliminar el material <b>{{ $item->cod }}</b>?
            </div>
        @else
            ...
            <div class="text-muted">
                El material tiene {{ $item->getCantDetails() }} @if($item->getCantDetails()==1) detalle asociado. @else detalles asociados. @endif
            </div>
        @endif
    </div>
