{{Form::open(array('action'=>array('InvOutcomesController@destroy',code($outcome->id)),'method'=>'delete', 'onsubmit'=>'deleteboton.disabled = true; return true;'))}}
    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
    <div class="modal-status @if( $outcome->getCantDetails() == 0 ) bg-red @else bg-yellow @endif"></div>
    <div class="modal-body text-center py-4">
        ...
        @if( $outcome->getCantDetails() == 0)
            ...
            <div class="text-muted">
                ¿Está seguro de eliminar la solicitud <b>{{ $outcome->cod }}</b>?
            </div>
        @else
            ...
            <div class="text-muted">
                La solicitud tiene {{ $outcome->getCantDetails() }} @if($outcome->getCantDetails()==1) detalle asociado. @else detalles asociados. @endif
            </div>
        @endif
    </div>
